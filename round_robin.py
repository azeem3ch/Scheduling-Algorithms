...
def chkwaiting(w_size,total_t,p_size,ready_list,waiting_list):
	loop=0
	while(loop<w_size):
	...
	return w_size,p_size,ready_list,waiting_list
		...
		if(ready_list[index].r_burst<=ready_list[index].quantum):
			if(ready_list[index].io!=1):
				for loop in range(ready_list[index].r_burst):
					...
					w_size,p_size,ready_list,waiting_list=chkwaiting(w_size,total_t,p_size,ready_list,waiting_list)
...
print ("process_id\tarrival_t\tburst_t    turnaround_t   waiting_t")
